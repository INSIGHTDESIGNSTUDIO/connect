...
export default function EditRolePage() {
  ...
  const fetchRole = async (roleId: string) => {
    ...
    try {
      ...
      // Convert to AdminRole type
      setRole(roleData as AdminRole);
    } catch (err) {
      console.error('Unexpected error while fetching role:', err);
      alert('An error occurred while loading the role');
      router.push('/admin/roles');
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (roleData: Omit<AdminRole, 'id'>) => {
    ...
    try {
      ...
      alert('Failed to update role. Please check the console for details.');
    } finally {
